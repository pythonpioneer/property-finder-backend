 ...
 * @param {Boolean} isNumeric - It takes a boolean value and states that the string is only number
 * @returns {Array} - It returns validation array to validate name fields.
 */
 const validateString = (messages, isOptional, length, isNumeric=false) => {
    ...
    return messages.map(name => {
        ...
        // if the string is only numeric
        if (isNumeric) validationChain.isNumeric();
